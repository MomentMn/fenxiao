...
import com.hansan.fenxiao.utils.*;
...
public class UserAction extends BaseAction {
    ...
    private ISupervisorService<Supervisor> supervisorService;
    ...
    public void userInfoJson() {
        ...
        User findUser = (User) this.userService.findById(User.class, loginUser.getUserId().intValue());
    ...
    public String levelList() {
        ...
        Set<Supervisor> levelList = this.supervisorService.levelList(userId, type);
    ...
    public void levelUserList() {
        ...
        //Integer  userId = 29;
        //下级总人数
        Set<Supervisor> levelUserList = this.supervisorService.levelUserList(userId);
        ...
        int one = LevelEnum.FIRST_LEVEL.getId();
        int two = LevelEnum.SECOND_LEVEL.getId();
        int three = LevelEnum.THIRD_LEVEL.getId();
        if (!CollectionUtils.isEmpty(levelUserList)) {
            for (Supervisor supervisor : levelUserList) {
                Integer level = supervisor.getLevel();
                if (one == level) {
                    firstLevelNum += 1;
                }
                if (two == level) {
